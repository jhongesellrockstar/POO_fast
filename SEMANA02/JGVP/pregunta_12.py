...
promedios = list(map(lambda alumno: sum(alumno.notas) / len(alumno.notas), alumnos))

alumnos_aprobados = list(
    filter(lambda datos: datos[1] >= 6, zip(alumnos, promedios))
)

promedios_dict = {
    alumno.nombre: promedio
    for alumno, promedio in alumnos_aprobados
}
